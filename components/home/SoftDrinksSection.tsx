...
const softDrinks = products
  .filter((p: any) => p.category && p.category.toLowerCase() === 'soft-drinks')
  .map((p: any) => ({
    ...p,
...
const SoftDrinksSection = () => {
  ...
  const totalTabs = Math.ceil(softDrinks.length / productsPerTab);
  ...
  const getCurrentTabProducts = () => {
    ...
    return softDrinks.slice(startIndex, endIndex);
  };
...
export default SoftDrinksSection;