...
const waterProducts = products
  .filter((p: any) => p.category && p.category.toLowerCase() === 'water')
  .map((p: any) => ({
    ...p,
...
const WaterSection = () => {
  ...
  const totalTabs = Math.ceil(waterProducts.length / productsPerTab);
  ...
  const getCurrentTabProducts = () => {
    ...
    return waterProducts.slice(startIndex, endIndex);
  };
