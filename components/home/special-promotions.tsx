      ...
      <div className="overflow-hidden">
        <div className={`transition-transform duration-500 tab-transform-${currentTab}`}>
          {promoTabs.map((tab, tabIdx) => (
            <div
              key={tabIdx}
              className={`grid gap-8 ${perTab === 1 ? 'grid-cols-1' : perTab === 2 ? 'grid-cols-1 sm:grid-cols-2' : perTab === 3 ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}`}
            >
              {tab.map((promo) => (
